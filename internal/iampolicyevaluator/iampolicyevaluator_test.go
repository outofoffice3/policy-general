...
func TestIAMPolicyEvaluatorPrivate(t *testing.T) {
	...
	iamPolicyEvaluator.CheckAccessNotGranted(config.Scope, iamPolicyEvaluator.GetRestrictedActions(), iamPolicyEvaluator.GetAWSClientMgr().GetAccountIds())
	...
	*/
	...
	totalCacheHits, ok := metricMgr.GetMetric(metricmgr.TotalCacheHits)
	assertion.True(ok)
	log.Printf("total cache hits [%v]\n", totalCacheHits)
}
